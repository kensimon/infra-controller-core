...
pub mod cmd {
        ...
        fn from(src: ManagedHostOutputWrapper) -> Self {
            ...
            if states[0] != dpu_state {
                let dpu_states = value
                    ...
                    .enumerate()
                    .map(|(i, x)| {
                        format!("DPU{}:{}", i, x.state.as_deref().unwrap_or("Unknown State"))
                    })
                    .collect::<Vec<String>>();
    ...
    async fn show_managed_hosts(
        ...
        match sort_by {
            SortField::PrimaryId => {
                managed_hosts.sort_by(|m1, m2| m1.machine_id.cmp(&m2.machine_id))
            }
        ...
        let mut data = vec![
            ...
            (
                ...
                Some(
                    m.quarantine_state
                        .map(|q| {
                            format!("yes (reason: {})", q.reason.as_deref().unwrap_or("<none>"))
                        })
    ...
    pub async fn show(
        ...
        let site_explorer_managed_hosts =
            api_client.get_all_explored_managed_hosts(page_size).await?;
