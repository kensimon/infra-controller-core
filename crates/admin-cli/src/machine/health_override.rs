...
pub mod cmd {
    ...
    use ::rpc::forge::{
        RemoveHealthReportOverrideRequest, {self as forgerpc},
    };
    use chrono::Utc;
    use health_report::{
        ...
        match command {
            Args::Show { machine_id } => {
                ...
                for r#override in response.overrides {
                    ...
                    let mode = match ::rpc::forge::OverrideMode::try_from(r#override.mode).map_err(
                        |_| CarbideCliError::GenericError("invalide response".to_string()),
                    )? {
