...
pub mod cmd {
    ...
    use ::rpc::forge::{
        Vpc, VpcsByIdsRequest, {self as forgerpc},
    };
    ...
    fn convert_instances_to_nice_table(instances: forgerpc::InstanceList) -> Box<Table> {
        ...
        for instance in instances.instances {
            ...
            let labels =
                crate::metadata::get_nice_labels_from_rpc_metadata(instance.metadata.as_ref());
    ...
    async fn show_instance_details(
        ...
        let instance = if let Ok(id) = MachineId::from_str(&id) {
            ...
            let instance_id = InstanceId::from_str(&id).map_err(|_| {
                CarbideCliError::GenericError("UUID Conversion failed.".to_string())
            })?;
